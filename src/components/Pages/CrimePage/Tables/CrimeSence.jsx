...
export default function CrimeSence({crimes, setCrimes}) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = crimes.map((n) => n.id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - crimes.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(crimes, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage,
      ),
    [order, orderBy, page, rowsPerPage, crimes],
  );
